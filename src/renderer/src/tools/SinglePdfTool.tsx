...
export function SinglePdfTool({
  ...
  else if (pdf.error && !lenient) reason = pdf.error.message
  else if (!pdf.handle && !pdf.error) reason = '正在读取文件…'
        ...
        <>
          ...
          <div className="single-file">
            <div className="big-thumb">
              {pdf.handle ? <PageThumb handle={pdf.handle} index={0} width={170} /> : pdf.error?.encrypted ? <Lock size={34} /> : pdf.error ? <AlertTriangle size={34} /> : null}
            </div>
