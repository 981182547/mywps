...
export type JobState =
  ...
  | { status: 'done'; outputs: string[]; notes: string[]; failures: string[]; preview?: string }
...
export function useJob() {
  ...
  const run = useCallback(async (job: Job) => {
    ...
    try {
      ...
      if (alive.current) setState({ status: 'done', outputs: r.outputs, notes: r.notes ?? [], failures: r.failures ?? [], preview: r.preview })
    } catch (e) {
